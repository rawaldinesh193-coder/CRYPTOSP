...
export class LedgerService {
  ...
  async executeTransfer(params: {
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (!sender || !sender.wallet) throw new Error('Sender user or wallet not found.');
      ...
      if (!recipientWallet || !recipientWallet.user) throw new Error(`Recipient wallet ID '${recipientWalletId}' not found.`);
      if (sender.wallet.walletId === recipientWalletId) throw new Error('Cannot transfer funds to your own wallet ID.');
  ...
  async executeAdminCredit(params: {
    ...
    if (amount.lessThanOrEqualTo(0)) throw new Error('Credit amount must be greater than zero.');
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (!user || !user.wallet) throw new Error('Target user or wallet not found.');
  ...
  async executeAdminReversal(params: {
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (originalTx.status === TransactionStatus.REVERSED) throw new Error('This transaction has already been reversed.');
      if (originalTx.status !== TransactionStatus.COMPLETED) throw new Error('Only COMPLETED transactions can be reversed.');
      if (!originalTx.senderId || !originalTx.recipientId) throw new Error('Transaction lacks sender or recipient details for reversal.');
      ...
      if (!senderWallet || !recipientWallet) throw new Error('Wallets not found for reversal.');
      ...
      if (!senderAccount || !recipientAccount) throw new Error('Accounts not found for reversal.');
