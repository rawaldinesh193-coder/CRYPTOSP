...
import { RegisterSchema, LoginSchema } from '@cryptosp/validation';
...
// Generate unique Cryptosp Wallet ID: CSP-XXXXXXXXXXXX
  ...
  try {
    ...
    if (!parse.success) {
      return res.status(400).json({
        success: false,
        error: { code: 'VALIDATION_ERROR', message: parse.error.errors[0].message, details: parse.error.format() },
      });
    }
    ...
    // Create User, Profile, Wallet, Accounts in PostgreSQL Transaction
